...
const EpisodesView: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingId) {
      ...
      setForm(EMPTY);
      setShowForm(false);
    } else if (plannedCuts > 0) {
      // Vai para etapa de sugestão com IA
      setStep('ai');
    } else {
      // Cadastro simples sem recortes
      addEpisode(form);
      setForm(EMPTY);
      setPlannedCuts(0);
      setCutTopics([]);
      ...
      <AnimatePresence mode="wait">
        {showForm && step === 'form' && (
          ...
          >
            ...
            {/* ── Seção de recortes (apenas na criação) ─────────────────── */}
            {!editingId && (
            ...
            <div className={styles.formActions}>
              ...
              {!editingId && plannedCuts > 0 ? (
                <button
                  type="submit"
                  className={styles.aiSubmitBtn}
                  disabled={!formIsReadyForAI}
                >
                  <Sparkles size={16} /> Sugerir agenda com IA
                </button>
              ) : (
                <button type="submit" className={styles.submitBtn}>
                  <Check size={16} /> {editingId ? 'Salvar' : 'Cadastrar'}
                </button>
              )}
            </div>
          </motion.form>
